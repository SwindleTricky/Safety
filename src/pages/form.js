...
const FormDetail = () => {
    ...
    async function getEmpDetail() {
      try {
        const [res1, res2] = await Promise.all([
          ...
          axios.get(api_emp + "/emp-info/" + data[0].ACTION_PIC),
        ]);
        setEmp(res1.data);
  ...
  //     let i = 0;
  //     picList.map((e) => {
  //       let emp_cd = picList[i].EMP_CD;
  //       let emp_name = picList[i].EMP_NAME;
  //       let section = picList[i].SECTION;
  //       options.push({
  //         value: i,
  //         empCode: emp_cd,
  //         empName: emp_name,
  //         label: emp_cd + " " + emp_name + " " + section,
  //       });
        ...
        axios
          ...
          .then((res) => {
            ...
            window.location.href = "./form?req_id=" + req_id.req_id[0];
              ...
              const message = res.data?.message || "Unauthorized: Please log in."; // Extract error message if available
              ...
              console.error(`Unexpected response status: ${res.status}`); // Handle other errors
            }
          });
      } catch (err) {
        console.log(err);
